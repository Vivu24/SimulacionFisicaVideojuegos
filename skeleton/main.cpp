...
Particle* anchor = nullptr;              // Part�cula fija (anclaje del muelle)
...
{    
    ...
    //// Crear la part�cula din�mica
    //Particle particula;
    //dynamicParticle = new Particle(particula);

    //// Crear la part�cula ancla (fija)
    //Particle anchorParticula;  // Usamos el constructor predeterminado
    //anchor = new Particle(anchorParticula);  // La part�cula ancla

    //// A�adir las part�culas al sistema
    //particleSystem->AddParticle(dynamicParticle);
    //particleSystem->AddParticle(anchor);  // A�adir tambi�n el ancla

    //// Crear el muelle entre la part�cula ancla y la part�cula din�mica
    //spring = new SpringForceGenerator(
    //    10.0f,  // Constante el�stica
    //    5.0f,   // Longitud en reposo
    //    anchor  // Part�cula fija (ancla)
    //);

    //// A�adir el generador de fuerza del muelle al sistema de part�culas
...
void keyPress(unsigned char key, const PxTransform& camera)
{
    ...
    {
    case ' ': {
        // Crear una nueva part�cula con el ESPACIO
        Particle* particle = new Particle(GetCamera()->getTransform().p, 10 * GetCamera()->getDir(), PxVec3(0, 0, 0), 1.0, 5.0);
        particle->SetAcceleration(PxVec3(0, -2.5, 0)); // Aceleraci�n debido a la gravedad
